...
| Artifact | Phase Created | Owner |
|---|---|---|
| `<project>/DESIGN.md` | Phase 0 | Human |
| `<project>/DESIGN-history.md` | Phase 0 | Human |
...
## Phase 0 — Ideation & Design Specification
...
### Rules
- The agent does **not** participate in Phase 0. This is a human design act.
- `DESIGN.md` is the only place scope is defined. If a feature is not in `DESIGN.md`, it
  does not exist.
- Amending `DESIGN.md` requires a deliberate decision and a dated changelog entry at the
...
- Known risk areas from `<project>/DESIGN.md`
...
### Inputs
- `<project>/DESIGN.md` and `<project>/ARCHITECTURE.md` (Phases 0–1, fully audited)
...
**`livery/standards/`** — three executable rule documents:
...
extend. Contains the generic TDD workflow, testing standards, Rust rules, and
anti-patterns. Contains no project-specific content.
...
all generic rules, TDD workflow, testing standards, Rust standards, anti-patterns,
and the naming-as-design-signal rule. References `livery/standards/` for detail.
...
2. Project reference document paths (pointing to both `livery/` and `<project>/`)
3. Crate/module responsibility boundaries for this project
4. Project-specific constraints (e.g., mint's offline requirement, credential protection)
5. The project-specific session contract commands
6. Project-specific anti-patterns
7. Key types quick-reference for the project
...
#### 2d. Initialise the Validation Script
...
- `<project>/DESIGN.md` and `<project>/ARCHITECTURE.md` (always in context)
...
- `prism stats --json` baseline from the previous session
...
1. **Agent reads context.** `livery/CLAUDE-base.md`, then `<project>/CLAUDE.md`,
   ...
   entries, the applicable skill file.
...
6. **Session log.** Agent writes a `<project>/SESSIONS.md` entry (see below).
...
- Before: [paste key prism stats --json metrics]
- After:  [paste key prism stats --json metrics]
...
`cargo test --workspace` and `prism check .`
...
**Key Prism commands:**
```bash
prism check . --strict --fix-suggestions   # Quality gate
prism audit .                              # Module depth report
prism stats . --json > .prism-baseline.json  # Save for delta tracking
prism map . --mermaid                      # Visual structure check
```

**Baseline delta tracking:** Compare `prism stats --json` output against the previous
session's saved baseline. Any metric that worsened requires a `<project>/SESSIONS.md`
note explaining why — or it must be fixed before the session closes.
...
`prism check --strict`, full coverage report, manual module-depth assessment.
...
> *v1.0 is defined in DESIGN.md, not discovered at the end.*
...
- [ ] All features listed in `DESIGN.md` v1 scope are implemented
...
- [ ] `prism check . --strict` passes with no failures
...
- [ ] `<project>/DESIGN.md` non-feature list is confirmed accurate
...
**On agentic tool plugins (Superpowers, etc.):** Use selectively. Recommended:
`sequential-thinking` for architectural reasoning before coding starts; `memory` for
persisting project context across long gaps between sessions. Avoid loading many plugins
simultaneously — prompt dilution reduces the influence of `livery/CLAUDE-base.md` and
...
3. **Subsystem-level Prism baselines.** Run `prism check <subsystem-path>` in addition to
   the workspace-level check. Subsystem quality can degrade even when workspace metrics
   look stable.
...
skills, base constitution, rationale docs — is written once and inherited by every
subsequent project. The project-specific harness — design, architecture, CLAUDE.md
extension, project skills, session log — is written fresh for each project, grounded in
the reusable methodology.

The pipeline runs in six phases. Phases 0 and 1 are purely human: writing the design
specification that bounds scope and the architecture document that defines structure.
These two documents are the upstream contracts that every subsequent session references.
Skipping them is the single most common cause of agentic codebases that grow brittle —
the agent never had a reliable model of what it was building. Phase 2 encodes the project
constitution: `livery/CLAUDE-base.md` and `<project>/CLAUDE.md` together give every
session the same design philosophy and coding rules; the skills directories give it the
same reusable procedures; `<project>/scripts/validate.sh` gives it the same definition
of done. Phase 3 is the repeatable unit of work: bounded sessions with structured opening
prompts, mandatory TDD discipline (no implementation before a failing test), and a session
log entry that records every decision made and every metric that moved. Sessions are small
by design — a large task that cannot be scoped to a session is a design problem, not a
scheduling problem.
